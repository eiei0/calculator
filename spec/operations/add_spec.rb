...
require_relative '../../lib/operations/add'

RSpec.describe Add do
  describe '#calculate' do
    let(:add_class) { described_class.new(first_int, second_int) }
    ...
    context 'when the left operand is positive' do
      ...
      context 'and the right is positive' do
        ...
        it { expect(add_class.calculate).to eq(15) }
      end

      context 'and the right is negative' do
        let(:second_int) { -5 }

        it { expect(add_class.calculate).to eq(5) }
      end

      context 'and the right is zero' do
        let(:second_int) { 0 }

        it { expect(add_class.calculate).to eq(10) }
      end
    end
    ...
    context 'when the left operand is zero' do
      ...
      context 'and the right is zero' do
        ...
        it { expect(add_class.calculate).to eq(0) }
      end
    end
